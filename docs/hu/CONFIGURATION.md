...
| `POSTGRES_USER`     | Docker   | `elyos`         | PostgreSQL felhasználónév (Docker Compose használja)                 |
| `POSTGRES_PASSWORD` | Docker   | `elyos123`      | PostgreSQL jelszó (Docker Compose használja)                         |
| `POSTGRES_DB`       | Docker   | `elyos`         | PostgreSQL adatbázis neve (Docker Compose használja)                 |
| `POSTGRES_PORT`     | Docker   | `5432`          | PostgreSQL host port (Docker Compose használja)                      |

> **Megjegyzés:** Docker Compose használatakor a `DATABASE_URL` automatikusan az egyes `POSTGRES_*` változókból épül fel. A `DATABASE_URL`-t csak akkor kell beállítanod, ha az ElyOS-t Docker-en kívül futtatod.
